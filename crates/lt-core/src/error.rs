...
mod tests {
    ...
    fn test_error_display() {
        let err = MurmurError::Audio("device not found".to_string());
        assert_eq!(err.to_string(), "Audio error: device not found");

        let err = MurmurError::Stt("connection failed".to_string());
        assert_eq!(err.to_string(), "STT error: connection failed");

        let err = MurmurError::Llm("CLI not found".to_string());
        assert_eq!(err.to_string(), "LLM error: CLI not found");

        let err = MurmurError::Permission("microphone denied".to_string());
    ...
    fn test_error_from_json() {
        ...
        let err: MurmurError = json_err.into();
        assert!(matches!(err, MurmurError::Json(_)));
    }

    #[test]
    fn test_error_from_toml() {
        let toml_err = toml::from_str::<toml::Value>("invalid = = toml").unwrap_err();
        let err: MurmurError = toml_err.into();
        assert!(matches!(err, MurmurError::Toml(_)));
    }
    ...
    fn test_result_type() {
        ...
        let err_result: Result<String> = Err(MurmurError::Audio("test".to_string()));
